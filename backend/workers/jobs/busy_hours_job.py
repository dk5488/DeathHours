...
from backend.service.business_service import get_all_businesses, BusinessService
...
def fetch_busy_hours_for_business(business_id: int, business_url: str, owner_id: int) -> dict:
    ...
    result = BusinessService.update_business_busy_hours(None, owner_id, business_id, hour_data)
    if result is None:
